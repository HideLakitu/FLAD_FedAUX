...
# FL hyper-parameters
PATIENCE = 25
CLIENT_FRACTION = 0.4  # Fraction of clients selected at each round for FedAvg-based approaches

MAX_ROUNDS = 10  # 手动设置总通信轮数
...
def trainClientModel(model, epochs, X_train, Y_train, X_val, Y_val, steps_per_epoch=None):
    ...
    loss_train = history.history['loss'][-1]  # -1表示取最后一个epoch的训练损失值
...
def FederatedTrain(clients, model_type, outdir, time_window, max_flow_len, dataset_name, epochs='auto', steps='auto',
    ...
    #  初始化记录表，设置字段名称，并创建输出文件以记录结果
    tuning_writer = csv.DictWriter(hyperparamters_tuning_file, fieldnames=tuning_fieldnames)  # 传入的是字典类型数据
    ...
    training_filename = model_name + '-epochs-' + str(epochs) + "-steps-" + str(steps) + "-trainingclients-" + str(
        training_mode) + "-weighted-" + str(weighted)
    ...
    while True:  # training epochs
        ...
        #  Stop if total rounds exceed MAX_ROUNDS
    ...
    training_file.close()  # 关闭文件对象，并释放系统资源
    shutil.move(outdir + '/' + training_filename + '.csv', outdir + '/' + training_filename + '-rounds-' + str(
        total_rounds) + '.csv')  # here we add the total training rounds to the filename
...
def assess_server_model(server_model, clients, update_clients=False, print_f1=False):
    ...
    for client in clients:
        ...
        #  对client的验证集 X_val 进行预测，将预测的概率转换为二进制标签，即判断该样本是否属于某一类
    ...
    # 清理会话, 释放内存空间，避免内存泄漏
    K.clear_session()
    gc.collect()  # 清除Keras相关的模型后，某些TensorFlow的张量可能仍未完全释放
...
#  将各client的模型参数按照样本数进行加权平均，然后更新server的global model
def aggregation_weighted_sum(server, clients, weighted=True):
    ...
    #  对于model的每一层weight,创建一个形状相同的零矩阵,用于后续的累加
